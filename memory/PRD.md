...
- **Aug 2026 (v1.0.53 BUILT & SUBMITTED)**: Fresh-food package — (1) on-brand no-barcode copy ("No barcode? Usually a great sign 🥦"); (2) quick-pick chips (Bakery/Deli meat/Cheese/Fruit & veg) → POST /api/analyze/name (verified: fruit&veg 10 Whole Food, deli meat 3 UPF); (3) 📸 photo AI scan → POST /api/scan/photo (GPT-4o vision, verified live: spag bol → 7/10 typical, assumptions shown, refinement chips Homemade 7/Jarred 5, low-confidence returns status "unclear"); honest-estimate design: is_estimate + assumptions[] + refinements[] in analysis, frontend renders estimate card with tappable refine chips (result.tsx). Photo scans available to ALL tiers, count against 5-scan free cap (premium gating decision deferred). expo-image-picker added. iOS build 54 uploaded to ASC (user must Submit for Review); Android AAB (versionCode 53): https://expo.dev/artifacts/eas/Tga_JnugUP348GNn4y37oEBwJdZOUWqW861gK5WtssU.aab. Bug fixed during dev: missing `import json` in backend (photo endpoint 500).
- **Aug 2026**: Subscription Activity panel (live, verified): subscription_events collection logs subscribed/renewed/resubscribed/plan_change/cancelled/expired/billing_issue from RevenueCat (apple/google) + Stripe webhooks. Dashboard feed with churn summary + days-subscribed on cancellations. Endpoint: GET /api/admin/subscription_events. History starts Aug 2026 (no backfill possible). Two demo events on test.screenshot account left in feed.
- **Aug 2026**: Dashboard/admin batch (all live, verified): platform detection (iOS/Android via User-Agent on login+scan, users.platform), per-user failed scan counts (signups table + User Admin), Recent Signups collapsed to latest 20 with expand-all button, full country names via Intl.DisplayNames, unlock error now distinguishes wrong password vs server redeploying, push single-email targeting, /api/admin/grant_scans (bonus scans). Sent apology pushes: Amy Hiatt ✅ delivered +3 scans; Stefy no push token (+3 scans granted, founder to email manually).
- **Aug 2026**: HARD CAP AT 5 free scans (live, verified): quick-scan limit changed from soft-5/hard-10 to hard 5. Free user at limit gets 403 → app shows existing paywall alert with one-tap purchase. Premium unaffected. 24+ free users at 6-9 scans now hit paywall on next scan. No app update needed. NO bonus scans (user decision — discussed dead-end exemption & teach-for-scan bonus, declined).
- **Jun 2026 (Build 52 SHIPPED)**: v1.0.52 built + iOS uploaded to App Store Connect (processing; user must submit for review/release in ASC). Android AAB delivered for Play Console upload: https://expo.dev/artifacts/eas/fgoWfkADRsjWYL5l3DHpe4jTqDWlCvcKJ252SWh6B1M.aab. Contents: (1) scanner iOS autofocus re-trigger loop every 2s (fixes small/glossy barcodes never focusing — Crosta Mollica bread packs reported by Michelle); (2) teach-the-app card v2 — hero placement for unknown products (meaningless 0-score ring hidden), auto-focused input, keyboard submit, tailored store-label copy. Also seeded 2 products for Macie in Malta via identify endpoint (Lay's Paprika Dolce 3/10, Kristal water 9/10) — both flipped to RECOVERED on dashboard. Annual/Family plans deferred to Build 53 (pending user console setup).
- **Jun 2026**: Store-label + case-code handling (live, verified): (1) GS1 restricted-range barcodes (2x-prefix EAN-13/UPC/EAN-8, incl. 02-padded) = in-store weighed/deli/reduced labels — now get an instant honest response + teach-the-app card, skipping the pointless AI identification (verified with real user scans 202051111586, 29496693). (2) 14-digit GTIN-14/ITF-14 case codes auto-convert to the contained EAN-13 and resolve normally (verified: Coke multipack code 15449000000993 → full 1/10 result). Regression clean (Peter's Yard 8/10). Root finding: ~half of "unknown product" scans were store labels, case codes, or new own-brand products (Lidl 4335619, new UK brand 5063446) absent from all global DBs.
- **Jun 2026**: Failed Scans panel upgrade: RECOVERED ✓ chip on lookups later resolved via teach-the-app or AI identification; header now shows "N unresolved / M logged / T attempts" (excludes internal + recovered). First organic teach-the-app use confirmed: real user karenashworth1991 scanned unknown Lidl own-brand water, taught the app, got 4/10 — flagged recovered.
- **Jun 2026**: Swap sanity fix (live, verified): products scoring 8+ get no healthier_alternatives; below 8, swaps must beat the product's final score (code gate + prompt rule). Repro: OJ from concentrate 9/10 was suggesting fresh-squeezed OJ also at 9/10 — now no swap; Coke 1/10 keeps its swap.
...
- Dashboard Tracked Links: copy-link buttons per code + TikTok USA ad link displayed (verified live, Aug 2026)
