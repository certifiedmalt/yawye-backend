...
- **Jun 2026**: Failed-scans fixes: backend rejects QR/URL/invalid barcodes with friendly message; AI-identification fallback no longer serves "Unknown product" with fake scores — now stores honest "could not identify" result; cache guard invalidates cached unknown-product entries. Frontend scan.tsx: removed qr/code128/code39 scanner types (ships with Build 51, NOT YET BUILT). Verified in production.
